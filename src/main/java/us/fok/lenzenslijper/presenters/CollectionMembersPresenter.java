...
public class CollectionMembersPresenter {
    ...
    public CollectionMembersPresenter(SchemaDictionary schema) {
        this.schema = schema;
    }

    public List<DocumentDetailView> makeViews(MemberDocumentsPageData data) {
    ...
    private DocumentDetailView configureMemberView(ProjectUserDocumentSummary member, Map<UUID, List<EnrolledAsset>> assetsByRevision, Map<UUID, List<ReleasedLinkSummary>> linksByRevision) {
        UUID revisionId = member.getRevisionId();
        return configureMemberView(member, assetsByRevision.get(revisionId), linksByRevision.get(revisionId));
    }
